    ...
    <div>
      ...
      {open && (
        <div className="mt-4 space-y-2">
          {/* Explain how the "Draw %" column behaves in the current mode. */}
          <p className="text-muted-foreground text-xs">
            {result.inputs.spendingMode === "rate"
              ? t.breakdown.rateNote(result.inputs.targetWithdrawalRatePct)
              : t.breakdown.amountNote}
          </p>
                ...
                {result.yearly.map((y) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <Badge
                        tone={
                          y.phase === "accumulation" ? "primary" : "neutral"
                        }
                      >
                    ...
                    <td className="text-negative px-3 py-2 text-right font-mono tabular-nums">
                      {y.withdrawals > 0
                        ? formatCurrency(y.withdrawals, c)
                        : "—"}
                    </td>
                    <td className="text-muted-foreground px-3 py-2 text-right font-mono tabular-nums">
